...
class XMiniGridADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            "state": np.zeros((self.seq_len, 5, 5, 2), dtype=int), # (seq_len, 5, 5, 2)
            "action": np.zeros(self.seq_len, dtype=int), # (seq_len,)
            "reward": np.zeros(self.seq_len), # (seq_len,)
            "done": np.zeros(self.seq_len, dtype=int), # (seq_len,)
            "target": np.zeros(self.seq_len, dtype=int), # (seq_len,)
        }
